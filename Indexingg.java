...
public class Indexingg {
	private static String indexPath = "C:\\\\Personal\\\\OVGU\\\\WiSe2020\\\\IR\\\\eclipse\\\\Lucene\\\\indexedFiles2" ;
	...
	private static void termVectors1() throws IOException {
		Directory dir = FSDirectory.open(Paths.get(indexPath ));
		...
		int maxDoc = reader.maxDoc(); 
		for (int i = 0; i < maxDoc; i++) {
			Terms termVector = reader.getTermVector( i  , "termV");
			Document d = reader.document(docId);
			String docPath = d.getFields().iterator().next().stringValue().toString();
	...
	private static void termVectors(int i) throws IOException {
		...
		//IndexableField content = reader.document(docId).getField("contents");
		//System.out.println(content);
		//list.add(content + "-");
				/*
				 * System.out.
				 * println("-----------------------Document List-----------------------"); int
				 * maxDoc = reader.maxDoc(); for (int i = 0; i < maxDoc; i++) {
				 */
		
		if (termVector != null && termVector.size() > 0) {
			
			Document d = reader.document(i);
			 String docPath = d.getField("path").stringValue();
			...
			while ((term = termsEnum.next()) != null) {
				...
				while ((docIdEnum = docsEnum.nextDoc()) != DocIdSetIterator.NO_MORE_DOCS) {
					...
					//System.out.println("term "+term.utf8ToString()+ " " + docId + " " + docsEnum.freq());
					//list.add(String.valueOf(docId));
					list.add(term.utf8ToString());
					//list.add(String.valueOf(docsEnum.freq()));
				}
			}
		}


		FileWriter csvwriter = new FileWriter(new File("corpusVectors.csv"), true) ;

		System.out.println(list);

		System.out.println("Term vector : " + termVector);
		StringBuilder sb = new StringBuilder();
		writeINexcel(list, csvwriter, sb);
		docId ++;
